...
on seven chapters.
 ...
 6. [[*Recursion][Recursion]]: describes fixpoint operators and implements recursion.
 7. [[*Types][Types]]: describes the simply-typed \lambda-calculus and its logical interpretation.

* The \lambda-calculus
** The untyped \lambda-calculus
The untyped [[https://en.wikipedia.org/wiki/Lambda_calculus][*λ-calculus*]] (or simply, *\lambda calculus*) is a logical formal system
which is also a model of computation (like the [[https://en.wikipedia.org/wiki/Turing_machine][Turing machines]], for
example). It was introduced in the 1930s by [[https://en.wikipedia.org/wiki/Alonzo_Church][Alonzo Church]], who was
looking for a formal system able to express the notion of
computability. It is [[https://en.wikipedia.org/wiki/Turing_completeness][Turing-complete]], that is, it has the property
...
Its [[https://en.wikipedia.org/wiki/Lambda_calculus_definition#Standard_definition][syntax]] is very simple; any expression is either:

  - a *variable*, $x,y,z,\dots$ which could have appeared before on
 ...
 * /function application/, a function taking a function $f$ and a term
   $x$ as arguments and returning $f(x)$.

 * /swapping of arguments/, a function that takes a function on two
...
[[https://en.wikipedia.org/wiki/Boolean_algebra][boolean logic]]. In boolean logic, we have two elements =true= and
=false=; and some logical gates such as
...
a program =(if ... then ... else ...)=. We are going to use this intuition to
...
*Exercise 2.3:* Compute the following logic clauses using lambda calculus
 ...
 - $(\mathtt{true} \vee \mathtt{false})\to \mathtt{false}$.
 - $\mathtt{false} \to (\mathtt{false} \to \mathtt{false})$
 - $(\neg \mathtt{false} \wedge \neg \mathtt{true}) \to \mathtt{true}$
