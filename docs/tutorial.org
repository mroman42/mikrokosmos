...
\lambda-calculus using the *[[file:index.org][Mikrokosmos]]* interpreter. It is divided
on seven chapters of progressive difficulty.
 
 1. [[*The \lambda-calculus][The \lambda-calculus]]: describes the lenguage.
 2. [[*Boolean logic][Boolean logic]]: implements logical gates.
 3. [[*Numbers][Numbers]]: implements aritmetical operators and comparisons.
 4. [[*Combinatory logic][Combinatory logic]]: describes the SKI combinators.
 5. [[*Data structures][Data structures]]: implements lists and binary trees.
 6. [[*Recursion][Recursion]]: describes fixpoint operators and recursion.
 7. [[*Types][Types]]: describes the simply-typed \lambda-calculus and its logical interpretation.

* The \lambda-calculus
** The untyped \lambda-calculus
The *untyped [[https://en.wikipedia.org/wiki/Lambda_calculus][λ-calculus]]* (sometimes called *λ-calculus*) is a formal
system that is also a model of computation (in the same sense that
[[https://en.wikipedia.org/wiki/Turing_machine][Turing machines]] are, for instance).  In fact, it is [[https://en.wikipedia.org/wiki/Turing_completeness][Turing-complete]],
...
on the untyped λ-calculus.  It was introduced in the 1930s by [[https://en.wikipedia.org/wiki/Alonzo_Church][Alonzo
Church]], who was looking for a formal system able to express the notion
of computability.

Its [[https://en.wikipedia.org/wiki/Lambda_calculus_definition#Standard_definition][syntax]], however, is very simple; any expression is either:

  - a *variable*, $x,y,z,\dots$ which may appear free or as a placeholder
    when defining a function;
    
  - a *lambda abstraction* $\lambda x. -$, which can be interpreted 
    as a function taking $x$ as an argument and returning what is
    written under $-$, which in turn can depend on $x$;
    
  - or an *application* of two expressions $(\lambda x. M)\ N$ which
    applies the function $(\lambda x. M)$ over the argument $N$.  When
    a function is applied, all the ocurrences of $x$ on $M$ will be
    substituted by $N$. This process is called a *\beta-reduction*, and
    it captures our usual notion of function application.
...
numbers, addition or multiplication will be topics for the next
chapters. An introduction to the untyped lambda calculus can be
...
[[https://github.com/mroman42/mikrokosmos][Mikroskosmos]]; which can be used directly from the browser. Every time
...
example, the evaluation of =id id= to =id= has followed this chain of
...
second one. For example, the =discard = const id= function is a function that
takes only one argument, discards it, and outputs =id=. This is called
 ...
 * /function application/, a function taking a function $f$ and a term
   $x$ as arguments and returning $f(x)$; and

 * /flip/, a function that takes a function on two
   arguments $f$ and returns a $g$ such that $g(x,y) = g\ x\ y = f\ y\ x = f(y,x)$.
   In other words, $\mathsf{flip}\ f\ x\ y = f\ y\ x$.
...
[[https://en.wikipedia.org/wiki/Boolean_algebra][boolean logic]]. In boolean logic, we have two elements =true= and =false=;
and some logical gates such as

  * the *and* gate, which outputs true if and only if its two inputs are
    true.
  * the *or* gate, which outputs true if any of its inputs is true, or
...
a program =(if ... then ... else ...)=. We are going to use this
...
In particular, =true= is exactly the same lambda term as =const=. This is
a very usual phenomenon in \lambda-calculus, where the same function
...
If we really want to write an =if-else= function, it will be, quite
...
they will be surprisingly easy to construct if we think of booleans as
functions choosing from two terms.
...
*Exercise 2.1:* Explain why this definition of the =and= gate works.
...
The question is now how can we encode them on \lambda-calculus. We
cannot simply write the two constructors on \lambda-calculus, but we
can make the natural numbers depend on any two given constructors.
This is again the same idea we used when we worked with boolean values:
we do not care about the structure, but about how can it be used later.
...
argument $b$.  Interpret the following two evaluations.
...
How to compute the predecessor of a number? We have not encoded
...
expected something easier! Stephen Kleene, who was a student of Alonzo
Church, discovered for the first time how to write a predecessor on
lambda calculus while at the dentist. This discovery made Church start
...
be found [[https://en.wikipedia.org/wiki/Church_encoding#Derivation_of_predecessor_function][here]]; but we will also study it in the *Exercise 3.5*.
...
which we will call =inc f=. It is a sort of reversed composition.
Study what happens when we compose =inc f= with itself and reimplement
=pred= using =inc=.
...
It is built by applying a =const false= function =n= times to a true
...
* Data structures
** Tuples
Tuples can be easily defined from boolean logic. The main idea will be
...
As seen in the example, we use =true= and =false= to select the first
...
In the following example, we define the =fix= combinator; which we
...
do this, we use the Mikrokosmos =!== operator, which binds an
...
# Recursion can easily lead to non-terminating computations.
# If Mikrokosmos enters an infinite loop, please reload this page.
# Your browser will probably warn you of a non-terminating script.</script>
</div>
#+END_EXPORT
...
For instance, =fix= is a non-terminating term (it *diverges*); but if
...
*Exercise 7.1:* The library =and= function is not typeable. Think why and
write a similar typeable =and= function.
 ...
 * The /product/ type =A × B= is the type of the pairs of elements
   where the first one is of type =A= and the second one is of type =B=.

 * The /union/ type =A + B= is a type that can contain an element of
   type =A= or an element of type =B=. It is a labeled disjoint union
   of the two types.

 * The /unit/ type =⊤= has only one element, called =unit= or =★=.

 * The /void/ type =⊥= has no elements. Its utility will be discussed
   later.
...
As an example, we could use the type =⊤ + ⊤= to write a new version
of the booleans. This new version will use =caseof= as its =ifelse=.
...
** Conjunctions, disjunctions and proofs
...
it is possible to write an empty function. The =abort= function is
useful if we want to use the [[https://en.wikipedia.org/wiki/Principle_of_explosion][principle of explosion]], that is, from
...
tree of any of your proofs with =@@=. For example, we will prove
that
...
to check the derivation tree of a given \lambda-term using =@=.  And
it can also simplify the derivation using =@@=, as we did before. We
will use our previously defined function/proof =swap= as an example.
