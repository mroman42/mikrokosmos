...
stdlibraries = unlines
  ...
  , "fold   = \\o.\\n.\\l.(l o n)"
  , "head = fold const nil"
  , "tail = \\l.first (l (\\a.\\b.tuple (second b) (cons a (second b))) (tuple nil nil))"
  ...
  , "naturals := fix (compose (map (plus 1)) (cons 0))"
  , "pair = \\x.\\y.(x,y)"
  , "fst = \\x.FST x"
  , "snd = \\x.SND x"
  , "inl = \\x.INL x"
  , "inr = \\x.INR x"
  , "caseof = \\x.\\y.\\z.CASE x OF (\\a.y a);(\\a.z a)"
  , "unit = UNIT"
  , "abort = \\x.ABORT x"
  , "absurd = \\x.ABSURD x"
  ]
